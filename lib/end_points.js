...
var end_points = {
	...
	updates: function(html){
		var $ = cheerio.load(html);
		var updates = [];
		$('.js-project-updates-content .timeline__item').each(function(i, elem){
			updates.push({
				id: i,
				date: $(this).find('.grid-post__date').text(),
				title: $(this).find('.grid-post__title').text(),
				content: $(this).find('.grid-post__content').text().replace(" Read more", ""),
				link: 'http://www.kickstarter.com' + $(this).find('.link').attr('href')
			});
		})
		return updates;
	}
